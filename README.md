...
지구를 탈출하여 우주로 올라가는 여정을 담은 **3D 캐주얼 플랫폼 게임**입니다. 3인 팀 프로젝트에서 플레이어 조작, 적 AI, 환경 기믹, UI를 담당했으며, Unity 기반의 플랫폼 게임 핵심 시스템을 구현하는 데 집중했습니다.
...
* **원인 분석:** 플레이어와 무빙 플랫폼 오브젝트가 유니티 런타임 상에서 서로 독립적인 트랜스폼(Transform) 좌표 연산을 수행하고 있어 논리적 동기화가 이루어지지 않았습니다. 플레이어와 무빙 플랫폼이 서로 독립적으로 좌표를 갱신했기 때문에 플랫폼의 이동이 플레이어에게 자연스럽게 전달되지 않았습니다.
...
* **결과:** 플레이어가 플랫폼의 이동을 안정적으로 따라가도록 개선하여 미끄러짐과 추락 문제가 해결되었습니다.
...
GTen은 Unity로 진행한 첫 팀 프로젝트였습니다. 개인 프로젝트와 달리 역할을 분담하여 기능을 개발하고 통합하는 과정을 경험하며 기능 구현뿐 아니라 **협업과 커뮤니케이션의 중요성**을 배울 수 있었습니다. 또한 플레이어, AI, 환경 기믹을 구현하면서 컴포넌트 간 의존성이 증가하는 문제를 경험했고 기능 구현에 집중했던 초기 프로젝트인 만큼 시스템 구조를 함께 고려하는 것의 중요성을 깨닫게 되었습니다.

이 경험은 이후 프로젝트에서 더 구조적인 설계를 고민하는 계기가 되었습니다. **[Shadow of the Dragon](https://github.com/solbinjung/unity-shadow-of-the-dragon)** 에서는 ScriptableObject 기반 데이터 관리와 중앙 매니저 구조를 적용하며 시스템을 개선했고, **[Project WHISPER](https://github.com/solbinjung/ue5-project-whisper)** 에서는 Unreal Engine의 Gameplay Framework를 활용해 역할과 책임을 분리한 구조를 설계하며 한 단계 더 발전된 방식으로 프로젝트를 진행할 수 있었습니다.

GTen은 기술적으로는 가장 초기의 프로젝트였지만 시스템 설계의 필요성을 직접 경험하며 이후 개발 방향에 큰 영향을 주었습니다.
